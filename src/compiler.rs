...
impl<T: Iterator<Item = char>> Compiler<T> {
    ...
    pub fn compile(mut self) -> Result<(Vec<Token>, PartialsMap)> {
        ...
        for name in partials.into_iter() {
            let path = self.ctx.template_path.join(
                &(name.clone() + "." +
                      &self.ctx.template_extension),
            );
...
mod tests {
    ...
    fn compile_str(template: &str) -> Vec<Token> {
        ...
        let (tokens, _) = Compiler::new(ctx, template.chars()).compile().expect(
            "Failed to compile",
        );
        tokens
    }
    ...
    fn test_compile_sections() {
        ...
        check_tokens(
            compile_str("before {{^name}}{{/name}} after"),
            &[
                Token::Text("before ".to_string()),
                Token::Section(
                    vec!["name".to_string()],
        ...
        check_tokens(
            compile_str("before {{#a}} 1 {{^b}} 2 {{/b}} {{/a}} after"),
            &[
                Token::Text("before ".to_string()),
                Token::Section(
                    vec!["a".to_string()],
